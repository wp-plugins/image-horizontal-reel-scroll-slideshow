...
function Ihrss() 
{
	...
	if ( ! empty($data) ) 
	{
		foreach ( $data as $data ) 
		{
			...
			$Ihrss_package = $Ihrss_package ."IHRSS_SLIDESRARRAY[$cnt]='<a title=\"$Ihrss_title\" href=\"$Ihrss_link\"><img alt=\"$Ihrss_title\" src=\"$Ihrss_path\"></a>';	";
			$cnt++;
		}
	}	
	?>
...
function Ihrss_install() 
{
	...
	if($wpdb->get_var("show tables like '". WP_Ihrss_TABLE . "'") != WP_Ihrss_TABLE) 
	{
		...
		$sSql = $IsSql . " VALUES ('http://www.gopiplus.com/work/wp-content/uploads/pluginimages/250x167/250x167_2.jpg' ,'http://www.gopiplus.com/work/2011/05/08/wordpress-plugin-image-horizontal-reel-scroll-slideshow/', '_blank', 'Image 2', '2', 'YES', 'gallery1', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
		
		$sSql = $IsSql . " VALUES ('http://www.gopiplus.com/work/wp-content/uploads/pluginimages/250x167/250x167_3.jpg', 'http://www.gopiplus.com/work/2011/05/08/wordpress-plugin-image-horizontal-reel-scroll-slideshow/', '_blank', 'Image 3', '1', 'YES', 'gallery1', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
...
function Ihrss_control() 
{
	...
	echo '<a target="_blank" href="http://www.gopiplus.com/work/2011/05/08/wordpress-plugin-image-horizontal-reel-scroll-slideshow/">Click here</a> for more help.<br>';
}
...
function Ihrss_admin_options() 
{
	...
	echo "<div class='wrap'>";
	echo "<h2>"; 
	echo wp_specialchars( "Image horizontal reel scroll slideshow" ) ;
	echo "</h2>";
	$Ihrss_title = get_option('Ihrss_title');
	$Ihrss_sliderwidth = get_option('Ihrss_sliderwidth');
	$Ihrss_sliderheight = get_option('Ihrss_sliderheight');
...
add_filter('the_content','Ihrss_Show_Filter');

function Ihrss_Show_Filter($content)
{
	return 	preg_replace_callback('/\[HR_IMAGE_SHOW:(.*?)\]/sim','Ihrss_Show_Filter_Callback',$content);
}

function Ihrss_Show_Filter_Callback($matches) 
{
	global $wpdb;
...
function Ihrss_init()
{
	...
	{
		register_sidebar_widget('Image horizontal reel scroll slideshow', 'Ihrss_widget');
	}
	
	if(function_exists('register_widget_control')) 
	{
		register_widget_control(array('Image horizontal reel scroll slideshow', 'widgets'), 'Ihrss_control');
	} 
}
